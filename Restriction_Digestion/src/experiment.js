...
checkHit = function( evt, box_array ) {
	for ( var i = 0; i < box_array.length / 2; i++ ) {
		var pt = box_array[ i ].globalToLocal( restriction_digestion_stage.mouseX, restriction_digestion_stage.mouseY );
		if ( box_array[ i ].hitTest( pt.x, pt.y ) ) {
			if ( i == ( string.indexOf( txtSequenceText1 ) ) ) {
			//change the color of the text to white if stageX touches on the sequence
				changeColor( i );
				break;
			}
		}
...
checkSissorsHit = function( scissors, scope ) {
		for ( var i = 0; i < GreenLineArr.length; i++ ) {
			var pt = scissors.localToLocal( 100, 0, GreenLineArr[ i ] );
			if ( GreenLineArr[ i ].hitTest( restriction_digestion_stage.mouseX, restriction_digestion_stage.mouseY ) ) {
				cut_flag = true;
